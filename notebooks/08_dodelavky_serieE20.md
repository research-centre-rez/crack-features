...
OUT_DIR = "/Users/gimli/cvr/data/microscopy/E20_results"
...
```python
# Reduce images to BSE images of E20
tifs = [tif for tif in tifs if "E20" in tif and "BSE images" in tif]
```
...
```python
file = tifs[0]
tif = imageio.imread(file).astype(float)
tif = ((tif - np.min(tif)) / (np.max(tif) - np.min(tif)) * 255).astype(int)
